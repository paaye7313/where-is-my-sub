# 내구독어디가 (where-is-my-sub) 프로그래스

> 완료된 작업과 앞으로 진행할 작업을 관리하는 문서. 오버뷰 문서(`where-is-my-sub_overview.md`)와 함께 사용.
> 마지막 업데이트: 2026-07-21

---

## ✅ 완료된 작업

### 대시보드
- [x] 구독 등록, 수정, 삭제, 검색
- [x] 구독 카드 클릭 시 수정/삭제 버튼 노출 (모바일 UX)
- [x] 드래그 앤 드롭 순서 변경 (PC + 모바일 터치, TouchSensor 적용)
- [x] 순서 수정 모드 (이름순/금액순/결제일순 정렬 + 저장/취소)
- [x] 월간/연간 구독 구분 표시
- [x] 상단 요약 카드 (이번 달 지출, 연간 환산, 구독 수)
- [x] PC 최대 너비 800px 중앙 정렬

### 지출 분석
- [x] 구독별 지출 비중 바 차트 (월간/연간 토글)
- [x] 구독 색상 차트 반영
- [x] 필터 토글 버튼 (구독 제외/포함)
- [x] 월/연간 환산 비교 테이블 (정렬 가능)
- [x] 가장 비싼 구독 표시
- [x] 반응형 레이아웃 (4열 → 3열 → 2열)

### 구독 추가/수정 모달
- [x] 빠른 선택 템플릿 12종
- [x] 이모지 아이콘 선택 (16개)
- [x] 색상 선택 (16가지)
- [x] 연간 결제 시 결제월 입력 필드
- [x] 모달 너비 최적화 (520px)

### 인증
- [x] 회원가입 / 로그인 (JWT, 7일 유효)
- [x] 로그아웃
- [x] localStorage 토큰 저장 + axios interceptor 자동 첨부

### 인프라 / 백엔드
- [x] Vercel 프론트 배포
- [x] Render 백엔드 배포
- [x] Render PostgreSQL 연동
- [x] API 엔드포인트 정리 (`/auth`, `/subscriptions`)
- [x] 로컬/Render DB 스키마 동기화 원칙 수립
- [x] 서버 웨이크업 배너 구현 (Render 콜드 스타트 대응)
  - 상단 배너 방식, 2초 이상 응답 없으면 노출
  - 서버 준비 완료 시 초록색 전환 후 2초 뒤 자동 숨김
  - 서버 연결 실패 시 로그인 에러 메시지 대신 배너가 안내
  - 로그인/대시보드 양쪽 화면 모두 적용
- `server/.env` 및 문서 내 실제 자격증명(`JWT_SECRET`) 노출 제거
  - git 추적 해제 + `.gitignore` 반영, `git-filter-repo`로 전체 히스토리에서 제거 후 force-push
  - 로컬 `JWT_SECRET` 랜덤 값으로 교체, `server/.env.example` 추가
  - 문서(`README.md`, `_overview.md`)에 박혀있던 예시 시크릿 값도 플레이스홀더로 교체
- Docker Compose로 로컬 개발 환경 구성 (프론트+백엔드+PostgreSQL)
  - 새 PC에서 PostgreSQL 재설치 없이 바로 개발 시작 가능하도록
  - `server/db/init.sql`로 최초 기동 시 스키마 자동 생성
  - `docker compose up --build`로 회원가입~구독 생성까지 전체 플로우 검증 완료

---

## 🚧 진행 중 / 다음 작업

### 🔴 최우선

### 🟡 문서/운영 정비
- [x] Render `JWT_SECRET` 교체 (2026-07-21) — 로컬 노출값과 동일했던 것 확인, 새 랜덤 값으로 교체 후 재배포 완료
- [ ] postgres가 켜져 있는 환경에서 로컬 DB 비밀번호(`1234`)도 교체
- [x] 배포 프로세스 확인 (2026-07-21) — Vercel/Render 둘 다 GitHub main 브랜치 push 시 자동 배포됨을 대시보드에서 확인
- [ ] Node.js / 주요 패키지 버전 명시
- [ ] CORS 설정 위치 및 허용 도메인 문서화
- [ ] 브랜치 전략 정리 (main 직접 커밋 여부)
- [ ] README 지속 업데이트

### 🟢 향후 아이디어 (우선순위 미정)
- [ ] 광고(Google AdSense) 삽입 검토 — 로그인 필요 서비스라 승인 까다로울 수 있음, 랜딩페이지 필요
- [ ] 배포 안정성 개선 — Render 슬립모드/DB 90일 만료 이슈 근본 대응 (유료 전환 or 대체 서비스 검토)

---

## 📝 작업 히스토리 (완료 시 여기로 이동)

| 날짜 | 작업 | 비고 |
|---|---|---|
| 2026-07-06 | 서버 웨이크업 배너 구현 | 배너 노출 → 준비 완료 표시 → 자동 숨김, 연결 실패 시 에러 메시지 제거 |
| 2026-07-21 | `server/.env` 노출 제거 (git 히스토리 정리, force-push, 자격증명 로테이션) | public 저장소에 DB 계정/`JWT_SECRET`이 커밋되어 있던 것 발견. `git-filter-repo`로 전체 히스토리에서 제거 + 문서 내 예시 시크릿 값도 정리 |
| 2026-07-21 | Docker Compose 로컬 개발 환경 구성 | 새 PC로 개발 환경 옮기며 PostgreSQL 재설치 대신 도커화 진행. 프론트/백엔드/DB 3개 컨테이너, `docker compose up --build`로 전체 플로우 검증 |
| 2026-07-21 | Render `JWT_SECRET` 교체 | 로컬 노출값과 동일하게 쓰고 있던 것 확인, Render 대시보드에서 새 랜덤 값으로 교체 → 자동 재배포 완료 |
